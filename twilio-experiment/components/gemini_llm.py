...
class GeminiLLM(LLMBase):
    ...
    def _stt_preprocess_numeric_text(self, texto: str) -> str:
        """
        Further preprocesses text for STT artifacts, especially for number extraction.
        Assumes text might have already been through convert_numeric_words_to_digits.
        """
        texto_corrigido = texto 

        texto_condensado = texto_corrigido
        previous_text = ""
        while texto_condensado != previous_text:
            previous_text = texto_condensado
            texto_condensado = re.sub(r'(\d)\s(\d)', r'\1\2', texto_condensado)
        
        return texto_condensado

    def extract_package_number(self, texto_original_stt_processed: str) -> Optional[str]:
        print(f"DEBUG: extract_package_number - Input (after initial word-to-digit): '{texto_original_stt_processed}'")

        texto_processado = self._stt_preprocess_numeric_text(texto_original_stt_processed)
        print(f"DEBUG: extract_package_number - After STT specific preprocessing (condensing digits): '{texto_processado}'")
        ...
        for padrao_ctx in padroes_contextuais:
            match_ctx = re.search(padrao_ctx, texto_processado, re.IGNORECASE)
            if match_ctx:
                candidato_bruto_ctx = match_ctx.group(1)
                digitos_do_candidato_ctx = "".join(filter(str.isdigit, candidato_bruto_ctx))
                
                if re.fullmatch(r"\d{3,15}", digitos_do_candidato_ctx): # Expect 3-15 digits
                    print(f"INFO: Número de encomenda extraído (padrão contextual '{padrao_ctx}', apenas dígitos): {digitos_do_candidato_ctx}")
                    return digitos_do_candidato_ctx
                else:
                    print(f"DEBUG: Candidato contextual '{digitos_do_candidato_ctx}' (de '{candidato_bruto_ctx}') não validou como \\d{{3,15}}.")

        print(f"DEBUG: Buscando por sequências de dígitos genéricas (e.g., \\b\\d{{3,15}}\\b) em '{texto_processado}'")
        matches_genericos = re.findall(r"\b\d{3,15}\b", texto_processado) 
        ...
        print(f"INFO: Nenhum número de encomenda (3-15 dígitos) extraído de: '{texto_original_stt_processed}' (processado como '{texto_processado}')")
        return None
    ...
    def obtain_package_number_details(self, numero_encomenda: str) -> Optional[dict]:
        ...
        if numero_encomenda.isdigit():
            ...
            if chave_com_prefixo_ped in self.dados_pedidos_gcs:
                ...
                chave_prefixo_zeros = "000" + numero_encomenda
                if chave_prefixo_zeros in self.dados_pedidos_gcs:
                    print(f"INFO: Detalhes encontrados para a encomenda (chave com prefixo zeros): {chave_prefixo_zeros}")
                    return self.dados_pedidos_gcs[chave_prefixo_zeros]
