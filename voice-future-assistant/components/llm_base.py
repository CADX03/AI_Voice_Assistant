...
8. Fecho da Conversação: Pergunta se há mais alguma questão. Se não houver, despede-te de forma breve e simpática. Após despedires-te do cliente, gera um output JSON. O JSON deve vir sempre no final de tudo o resto, aquando do fecho da conversa. O ficheiro JSON deve seguir o seguinte formato:
```json
{
  "identificacao_cliente": {
    "numero_encomenda": "valor",
...
"""
    ...
    def check_final_response(self, response: str) -> tuple[bool, str, str]:
        pattern = re.compile(
            r'(?:```json)?[\s]*'  
            r'(\{[\s]*'           
            r'"identificacao_cliente":[\s]*\{[\s]*'
            r'"numero_encomenda":[\s]*(?:null|"[^"]*")[\s]*,[\s]*'
            r'"email":[\s]*(?:null|"[^"]*")[\s]*\}[\s]*,[\s]*'
            r'"resumo":[\s]*("[^"]*"|[^,}]+)[\s]*,[\s]*'  
            r'"tipificacao":[\s]*("[^"]*")[\s]*,[\s]*'    
            r'"redirecionamento":[\s]*(true|false)[\s]*'   
            r'\})'                                      
            r'[\s]*(?:```)?',    
            re.DOTALL
        )
